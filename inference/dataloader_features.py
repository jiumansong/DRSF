...
class PTFeatureDataset(Dataset):
            ...
            if os.path.isdir(class_path):
                for file in os.listdir(class_path):
                    if file.endswith(".pt"):
                        ...
                        label = int(class_folder.split("_")[-1])  # Parse the ID according to the specific situation
    ...
    def __getitem__(self, idx):
        ...
        return {
            "patches_features": patches_features,  # (patch_num, feature_dim)
            "label": label,  
...
class SlideBasedPTFeatureDataset(Dataset):
    def __init__(self, root_dir):
        """
        Input is a WSI containing one/multiple large-size images (png) for ensemble aggregation strategy evaluation
        Args:
            root_dir (str): The root directory containing patch features
        features/                 
        ├── test/                
    ...
    def __getitem__(self, idx):
        ...
        for file_path, _ in files:
            ...
            patches_features = data["patches_features"].squeeze(0)  # (patch_num, feature_dim)
        ...
        return {
            "patches_features": stacked_features, # (png_num, patch_num, feature_dim)
            "label": label,  
            "slide_name": slide_name,  
