...
# All rights reserved.
#
# 2025 Modified based on:
# Zhu, Lianghui, et al. "Vision mamba: Efficient visual representation learning with bidirectional state space model." 2024
# [Ref: https://github.com/hustvl/Vim]
...
from model.resnet50_layer3 import *
from model.resnet34 import *
from model.patch_embed_simple import *
from model.resnet50_layer2_pretrainedTCGA import *
from model.resnet50_layer4_pretrainedTCGA import *
...
__all__ = [
    'vim_png_onestage',
    'vim_patch_resnet34',
    'vim_patch_resnet50_3layers',
    'vim_patch_resnet50_2layers_pretrainedTCGA',
    'vim_patch_resnet50_4layers_pretrainedTCGA'
]
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        if if_abs_pos_embed:   
            self.pos_embed = nn.Parameter(torch.zeros(1, 21, self.embed_dim))
        
        if if_rope:   
            half_head_dim = embed_dim // 2  
            hw_seq_len = pt_hw_seq_len   
        ...
        if self.if_abs_pos_embed:   
            ...
            if self.pos_embed is None:
                self.pos_embed = nn.Parameter(
                    torch.zeros(1, M, self.embed_dim, device=x.device)
                )
                trunc_normal_(self.pos_embed, std=0.02)  

            if self.pos_embed.size(1) != M:
                old_pos_embed = self.pos_embed.data
                new_pos_embed = nn.functional.interpolate(
                    old_pos_embed.unsqueeze(0),            
                    size=(M, self.embed_dim),
                    mode='bilinear',
                ).squeeze(0)
